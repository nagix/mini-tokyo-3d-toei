 ...
 *
 *    https://github.com/nagix/mini-tokyo-3d-toei
 */
...
// Interval of refreshing bus positions in milliseconds
var BUS_REFRESH_INTERVAL = 15000;
...
var MAX_BUS_SPEED_KMPH = 30;

// Bus acceleration in km/h/s
var BUS_ACCELERATION_KMPHPS = 3;

var MAX_BUS_SPEED = MAX_BUS_SPEED_KMPH / 3600000;
var BUS_ACCELERATION = BUS_ACCELERATION_KMPHPS / 3600000000;
var MAX_BUS_ACCELERATION_TIME = MAX_BUS_SPEED / BUS_ACCELERATION;
var MAX_BUS_ACC_DISTANCE = MAX_BUS_ACCELERATION_TIME * MAX_BUS_SPEED / 2;
...
// API URL
var API_URL = 'https://api.odpt.org/api/v4/';
...
Promise.all([
	...
	loadJSON('data/busroute-patterns.json'),
	loadJSON('data/busstop-poles.json'),
	loadJSON('https://mini-tokyo.appspot.com/f')
]).then(function([
...
busroutePatternLookup = buildLookup(busroutePatternRefData);
busstopPoleLookup = buildLookup(busstopPoleRefData);
...
map.once('styledata', function () {
	...
	map.addLayer({
		...
		source: {
			type: 'geojson',
			data: filterFeatures(railwayFeatureCollection, function(p) {
				return p.zoom === undefined && p.type === 0 && p.altitude === 0;
			})
		},
		paint: {
			'line-color': ['get', 'color'],
			'line-width': ['interpolate', ['exponential', 2], ['zoom'], 11, ['/', ['get', 'width'], 2], 12, ['get', 'width']]
		}
	}, 'building-3d');
			...
			startAnimation({
				callback: function(elapsed, duration) {
					...
					Object.keys(activeBusLookup).forEach(function(key) {
						var car = activeBusLookup[key].car;
	...
	map.on('zoom', function() {
		...
		Object.keys(activeBusLookup).forEach(function(key) {
			updateBusShape(activeBusLookup[key]);
		});
	});
	...
	function updateBusProps(bus) {
		var busstopOffsets = bus.feature.properties['busstop-offsets'];
		var sectionIndex = bus.sectionIndex;
		var offset = bus.offset = busstopOffsets[sectionIndex];

		bus.interval = busstopOffsets[sectionIndex + bus.sectionLength] - offset;
	}

	function updateBusShape(bus, t) {
		var car = bus.car;
		var position, scale, userData, p, coord, bearing, mCoord;
		...
		p = getCoordAndBearing(bus.feature, bus.offset + bus._t * bus.interval, 1, 0)[0];
...
if (bus.sectionLength > 0 && bus.interval === 0) {
	console.log(bus.id, bus.fromBusstopPole, bus.toBusstopPole, bus.sectionIndex, bus.sectionLength, bus.feature.properties['busstop-offsets']);
			...
			function stand() {
				...
				if (setBusSectionData(bus, !realtimeBusLookup[bus.id])) {
					updateBusProps(bus);
					updateBusShape(bus, 0);
				} else {
					final = true;
				}
...
if (bus.sectionLength > 0 && bus.interval === 0) {
	console.log(bus.id, bus.fromBusstopPole, bus.toBusstopPole, bus.sectionIndex, bus.sectionLength, bus.feature.properties['busstop-offsets']);
				...
				if (bus.sectionLength > 0 && bus.interval > 0) {
					...
					bus.animationID = startAnimation({
						complete: !final ? stand : function() {
							stopBus(bus);
						},
						duration: 15000
					});
			...
			function repeat(elapsed) {
				setBusStandingStatus(bus, false);
				bus.animationID = startBusAnimation(function(t) {
					updateBusShape(bus, t);
				}, stand, Math.abs(bus.interval), 1, elapsed);
			}
	...
	function getLocalizedBusroutePatternTitle(busroute) {
		title = (busroutePatternLookup[busroute] || {}).title || {};
		return title[lang] || title['en'];
	}

	function getLocalizedBusstopPoleTitle(busstop) {
		title = (busstopPoleLookup[busstop] || {}).title || {};
		return title[lang] || title['en'];
	}
	...
	function setBusStandingStatus(bus, standing) {
		var departureTime = 
		...
		bus.description =
			...
			(bus.toBusstopPole ?
				'<br><strong>' + dict['next-busstop'] + ':</strong> ' + getLocalizedBusstopPoleTitle(bus.toBusstopPole) : '');
	}
	...
	function stopBus(bus) {
		stopAnimation(bus.animationID);
		trainLayers.removeObject(bus.car, 1000);
		delete bus.car;
	...
	function loadRealtimeTrainData() {
		Promise.all([
			loadJSON(API_URL + 'odpt:TrainInformation?'),
			loadJSON(API_URL + 'odpt:Train?')
	...
	function loadRealtimeBusData() {
		loadJSON(API_URL + 'odpt:Bus?odpt:operator=odpt.Operator:Toei').then(function(busRefData) {
			realtimeBusLookup = {};

			busRefData.forEach(function(busRef) {
				var id = removePrefix(busRef['owl:sameAs']);
				var busNumber = busRef['odpt:busNumber'];
				var busroutePattern = removePrefix(busRef['odpt:busroutePattern']);
				var toBusstopPole = removePrefix(busRef['odpt:toBusstopPole']);
				var fromBusstopPole = removePrefix(busRef['odpt:fromBusstopPole']);
				var fromBusstopPoleTime = busRef['odpt:fromBusstopPoleTime'];
				var feature = featureLookup[busroutePattern];
				var activeBus = activeBusLookup[id];

				if (activeBus) {
					activeBus.toBusstopPole = toBusstopPole;
					activeBus.fromBusstopPole = fromBusstopPole;
					activeBus.fromBusstopPoleTime = fromBusstopPoleTime;
				} else if (feature) {
					activeBus = {
						id: id,
						busNumber: busNumber,
						busroutePattern: busroutePattern,
				...
				realtimeBusLookup[id] = activeBus;
			});
...
function startBusAnimation(callback, endCallback, distance, timeFactor, start) {
	var maxSpeed = MAX_BUS_SPEED * timeFactor;
	var acceleration = BUS_ACCELERATION * timeFactor * timeFactor;
	var maxAccelerationTime = MAX_BUS_ACCELERATION_TIME / timeFactor;
	var duration = distance < MAX_BUS_ACC_DISTANCE * 2 ?
		Math.sqrt(distance / acceleration) * 2 :
		maxAccelerationTime * 2 + (distance - MAX_BUS_ACC_DISTANCE * 2) / maxSpeed;
	var accelerationTime = Math.min(maxAccelerationTime, duration / 2);
	...
	return startAnimation({
		callback: function(elapsed) {
			...
			if (elapsed <= accelerationTime) {
				...
				d = MAX_BUS_ACC_DISTANCE + maxSpeed * (elapsed - maxAccelerationTime);
			}
			callback(d / distance);
		},
		complete: endCallback,
...
function setBusSectionData(bus, final) {
	...
	return sectionIndex < finalIndex;
}
