...
var featureLookup = {};
var animationID = 0;
var stationLookup, railwayLookup, timetableLookup, railDirectionLookup, trainTypeLookup, operatorLookup, airportLookup, flightStatusLookup, busrouteLookup, busstopLookup;
...
Promise.all([
	...
	loadJSON('data-extra/busroute-patterns.json'),
	loadJSON('data-extra/busstop-poles.json'),
	loadRailwayRefData(),
	loadStationRefData(),
...
busstopPoleData.forEach(function(busstop) {
	merge(busstopLookup[busstop.id].title, busstop.title);
});
...
[13, 14, 15, 16, 17, 18].forEach(function(zoom) {
	...
	Object.keys(busstopGroups).forEach(function(name) {
		var feature = turf.union.apply(this, busstopGroups[name].map(function(id) {
			return turf.buffer(turf.point(busstopLookup[id].coord), unit / 4);
		}));
...
busroutePatternRefData.filter(function(busroute) {
	return busroute.coords;
}).forEach(function(busroute) {
	var id = busroute.id;
	var busrouteFeature = turf.lineString(busroute.coords, {
		id: id,
		type: 0,
		color: '#9FC105',
		width: 2,
	...
	featureLookup[id] = busrouteFeature;
});
...
function loadRailwayRefData() {
	return loadJSON(API_URL + 'odpt:Railway?').then(function(data) {
...
function loadStationRefData() {
	return loadJSON(API_URL + 'odpt:Station?').then(function(data) {
...
function loadTrainTypeRefData() {
	return loadJSON(API_URL + 'odpt:TrainType?').then(function(data) {
...
function loadBusroutePatternRefData() {
	return loadJSON(API_URL + 'odpt:BusroutePattern?odpt:operator=odpt.Operator:Toei').then(function(data) {
		return data.map(function(busroute) {
			return {
				id: removePrefix(busroute['owl:sameAs']),
				title: {ja: busroute['dc:title']},
				coords: (busroute['ug:region'] || {}).coordinates,
				busstops: busroute['odpt:busstopPoleOrder'].map(function(obj) {
					return removePrefix(obj['odpt:busstopPole'])
				})
			};
		});
	});
